...
from app.config import MAX_WAIT, INDEX_FW
...
class MotionControl(Module):
    """ 
    Class containing the motion control of the Thymio

    @variables:
    Var ctx: context of the application
    Var relative_distance: relative distance to the obstacles
    Var waypoint: next waypoint to reach
    Var times: time for the corners control
    Var factor: factor to increase the speed of the robot
    Var arrived: if the robot arrived to the next waypoint

    @functions:
    Func __init__: initiates the class
    Func setNewWaypoint: update the waypoint to go
    Func run: update function
    Func update_waypoint: update the waypoint to go
    Func update_motor_control: control function
    Func controlPosition: PD control with Fuzzy control for the angle
    Func controlWithDistance: reactive control
    """
    ...
    def setNewWaypoint(self, indexMore):  # update the waypoint to go
        ...
        index = min(self.ctx.state.next_waypoint_index + indexMore, len(self.ctx.state.path) - 1)
